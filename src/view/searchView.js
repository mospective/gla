import {elements, imgUrl, noCoverSVG} from '../base';

export const getInput = () => elements.inputSearch.value;

const renderSearch = game => {
    const markup = `
    <li class="scale-in-center ">
        <div class="game-card">
            <div class="card__img">
                ...
                <img src="${(game.cover === undefined ) || (!game.cover) ? noCoverSVG :imgUrl(game.cover.url)}" alt="${game.name}">
            </div>
...
export const clearResults = ()=> {
    elements.topPagination.innerHTML = '';
}
...
const createBtn = (page, type) => `
    ...
    <button class="top-pagination__btn top-pagination__btn--${type}" data-goto="${type === 'prev' ? page - 1: page + 1}">
                    <svg version="1.1" id="prev" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 407.436 407.436" style="enable-background:new 0 0 407.436 407.436;" xml:space="preserve"><polygon points="112.814,0 91.566,21.178 273.512,203.718 91.566,386.258 112.814,407.436 315.869,203.718 "/></svg>
...
const renderBtns = (page, numResult, resPerPage) => {
    ...
    elements.topPagination.insertAdjacentHTML('afterbegin', button);

    // let sm = window.matchMedia("(max-width: 540px)");
    // if (sm.matches) {
...
export const renderSearchResults = (games, page = 1, resPerPage = 5) => {
    // Render results for current pahe
    const start = (page - 1) * resPerPage;
    const end = page * resPerPage;

    games.slice(start, end).forEach(renderSearch);
    // console.log(games.length);
    renderBtns(page, games.length, resPerPage);
   
};


// export const renderSearchResults = (games) => {
//     // console.log('Render games args');
//     // console.log(games);
//     games.forEach(renderSearch);
// };
